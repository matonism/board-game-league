...
function InstallInstructions() {
    ...
    const [addToHomeScreen, setAddToHomeScreen] = useState('no add to homescreen');
    const [install, setInstall] = useState('no install');


    useEffect(() => {

        // Check for PWA support and installation status
        const deferredPrompt = window.deferredPrompt;
        if (deferredPrompt) {
            setInstall('We FOUND INSTALL');
            // setIsInstalled(deferredPrompt.userChoice === 'accepted');

            // Handle deferred prompt (for installability checks)
    ...
    function getPlatformSpecificPrompt() {
        if (isMobile()) {
            if (isIos()) {
                return (
                    ...
                    <div className='install-modal-content'>
                        ...
                        <div className='instruction-list-header'>To add this app to your home screen</div>
            ...
            //             <div className='instruction-list-item'>
            //                 <button onClick={() => window.addEventListener('beforeinstallprompt', (e) => e.prompt())}>
            //                     Install App
            //                 </button>
            //             </div>
            //         </div>
